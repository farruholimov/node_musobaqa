...
export default class TgBot {
    ...
    public async runBot() {
        ...
        this.router.route('phone', async (ctx) => {
            try {
                await this.usersController.setPhone(
                    ctx,
                    ctx.msg.contact
        ...
        this.router.route('brand_name', async (ctx) => {
            try {
                ...
                await this.msgController.removeInlineKeyboard(
                    ctx,
                    ctx.session.message_ids.ask_brand_name
                );
        ...
        this.router.route('address', async (ctx) => {
            try {
                ...
                await this.msgController.removeInlineKeyboard(
                    ctx,
                    ctx.session.message_ids.ask_address
                );
        ...
        this.router.route('address_target', async (ctx) => {
            try {
                await this.mastersController.setAddressTarget(
                    ctx,
                    ctx.msg.text
                );
                await this.msgController.removeInlineKeyboard(
                    ctx,
                    ctx.session.message_ids.ask_address_target
                );
        ...
        this.router.route('message_to_admin', async (ctx) => {
            try {
                await this.mastersController.saveAdminMessage(
                    ctx,
                    ctx.msg.text
                );
                await this.mastersController.sendFinalInfo(ctx);
        ...
        bot.on('callback_query:data', async (ctx) => {
            ...
            switch (command) {
                ...
                case 'set_section':
                    try {
                        await this.mastersController.setSection(
                            ctx,
                            query.section_id
                        );
                ...
                case 'my_timetable':
                    try {
                        await this.mastersController.sendDays(ctx, true);
                    } catch (error) {
                        console.log(error);
                    }
                    break;
                ...
                case "search_master":
                    try {
                        await this.msgController.sendMasterSearchMenu(ctx)
                    } catch (error) {
                        console.log(error);
                    }
                    break;
                ...
                case 'skip':
                    switch (query.next_step) {
                        ...
                        case 'address_target':
                            try {
                                await this.msgController.askAddressTarget(
                                    ctx,
                                    true
                                );
                                await this.usersController.updateStep(
                ...
                case 'back':
                    ...
                    switch (query.step) {
                        case 'waiting_verification':
                            try {
                                await this.mastersController.sendWaitingMenu(
                                    ctx,
                                    true
                                );
                                await this.usersController.updateStep(
                        ...
                        case 'master_menu':
                            try {
                                await this.mastersController.sendMainMenu(
                                    ctx,
                                    true
                                );
                                await this.usersController.updateStep(
                            ...
                            break;
                        case 'return_timetable':
                            try {
                                await this.mastersController.sendDays(
                                    ctx,
                                    true
                                );
                                await this.usersController.updateStep(
